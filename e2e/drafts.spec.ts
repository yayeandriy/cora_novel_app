...
// Utilities
async function createProject(page: Page, projectName: string = 'Test Project'): Promise<number> {
...
async function createDocument(page: Page, docName: string = 'Test Doc'): Promise<number> {
...
async function selectDocument(page: Page, docName: string): Promise<void> {
...
test.describe('Draft Feature E2E Tests', () => {
  ...
  test('should display draft styling correctly', async ({ page }) => {
    ...
    const bgColor = await draftItem.evaluate(el => 
      window.getComputedStyle(el).backgroundColor
    );
    expect(bgColor).toBeTruthy();
    
    // Verify border styling
    const borderLeft = await draftItem.evaluate(el =>
