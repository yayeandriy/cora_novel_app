...
describe('Drafts Error Reproduction Tests', () => {
  describe('Database Schema Mismatch - "no such column: name"', () => {
    it('should reproduce error when drafts table has wrong schema', async () => {
      ...
      const missingColumns = insertColumns.filter(
        col => !oldDraftsSchema.columns.includes(col)
      );
      ...
      console.log(
        'Error reproduction: Trying to insert into old schema would fail with:',
        `"no such column: ${missingColumns[0]}"`
      );
    });
